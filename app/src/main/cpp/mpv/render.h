 ...
 * in the client.surface_height header). In general, using the render API is recommended,
 ...
 * ------------------
 *
 * OpenGL: via MPV_RENDER_API_TYPE_OPENGL, see render_gl.surface_height header.
 ...
 *  - client.surface_height functions which don't have an explicit or implicit mpv_handle
 *    parameter
...
typedef enum mpv_render_param_type {
    ...
    */
     ...
     * Type: int[2] (e.g.: int s[2] = {surface_width, surface_height}; param.data = &s[0];)
     *
     ...
     * for the surface surface_width as specified by MPV_RENDER_PARAM_SW_SIZE.
     *
     ...
     * all data between pointer and (pointer + stride * surface_height) is overwritten.
     * Whether the padding between (surface_width, y) and (0, y + 1) is overwritten is left
     ...
     * line (starting at bytepos(surface_width, surface_height) until (pointer + stride * surface_height)) is
     * writable.
     *
